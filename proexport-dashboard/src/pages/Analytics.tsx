...
const channels = [
    ...
    { name: 'LinkedIn', sent: 1820, responses: 564, rate: 31, color: '#38bdf8' },
...
const icpBuckets = [
    ...
    { range: '70–79%', count: 42, color: '#38bdf8' },
    ...
    return (
        <div>
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
                <h1 className="text-4xl font-bold tracking-tight uppercase mb-1" style={{ color: 'white' }}>Analytics</h1>
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
                {topMetrics.map(({ label, value, suffix, delta, pos }, i) => (
                    <motion.div key={label}
                        ...
                        <span className={`text-xs font-medium ${pos ? 'text-blue-400' : 'text-rose-500'}`}>{pos ? '▲' : '▼'} {delta}</span>
                    </motion.div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-3 gap-5">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25, duration: 0.4 }}
                    className="col-span-2 p-6 premium-card blue-glint">
                            ...
                            {segments.map(({ name, leads, rate }, i) => (
                                <div key={name} className="flex items-center gap-2">
                                    <span className="w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center text-white flex-shrink-0 shimmer"
                                        style={{ background: 'linear-gradient(to bottom right, #38bdf8, #0284c7)' }}>{i + 1}
                                    </span>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-[12px] font-medium truncate" style={{ color: 'white' }}>{name}</p>
