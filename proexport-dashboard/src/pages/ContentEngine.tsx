...
const statusConfig: Record<string, { bg: string; text: string; border: string; icon: typeof Check }> = {
 published: { bg: '#effaf4', text: '#16784c', border: '#a7e3c0', icon: Check },
 draft: { bg: 'rgba(255,255,255,0.05)', text: '#64748b', border: 'rgba(255,255,255,0.08)', icon: FileText },
 scheduled: { bg: '#fef9ec', text: '#92640a', border: '#f6d993', icon: Clock },
}
 ...
 return (
  <div>
   <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
    <h1 className="text-2xl font-semibold mb-1" style={{ color: 'rgba(255,255,255,0.04)' }}>Content Engine</h1>
   ...
   <motion.div
    ...
    style={{
     background: 'linear-gradient(135deg, #94a3b8 0%, #ca8a04 100%)',
     boxShadow: '0 8px 28px rgba(31,41,51,0.20)',
    }}
   >
    ...
    {posts.map(({ title, desc, status, views, likes, comments }, i) => {
     ...
     return (
      <motion.div key={title}
       ...
       className="p-5 flex gap-4 items-start premium-card gold-glint">
       <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
       ...
       <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-4 mb-1">
         <h3 className="text-sm font-semibold" style={{ color: 'rgba(255,255,255,0.04)' }}>{title}</h3>
   ...
   <motion.button whileTap={{ scale: 0.97 }} className="btn-gold flex items-center gap-2 px-6 py-3">
