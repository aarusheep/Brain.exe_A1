...
const resultStyle: Record<string, { bg: string; text: string; border: string }> = {
  'Positive': { bg: '#effaf4', text: '#16784c', border: '#a7e3c0' },
  'Closed Won': { bg: 'rgba(255,255,255,0.03)', text: '#ca8a04', border: '#c8d5e0' },
  'Follow-up': { bg: '#fef9ec', text: '#92640a', border: '#f6d993' },
  ...
  return (
    <div>
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
        <h1 className="text-2xl font-semibold mb-1" style={{ color: 'rgba(255,255,255,0.04)' }}>Meetings</h1>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {statCards.map(({ label, value, suffix }, i) => (
          <motion.div key={label}
            ...
            className="p-5 text-center premium-card gold-glint">
        ...
        {upcoming.map((m, i) => (
          <motion.div key={m.title + m.company}
            ...
            <div className="flex items-start justify-between mb-3">
              <div>
                <h3 className="font-semibold text-sm" style={{ color: 'rgba(255,255,255,0.04)' }}>{m.title}</h3>
              ...
              <span className="flex items-center gap-1 text-[11px] font-medium px-2.5 py-1 rounded-full"
                style={{ background: 'rgba(255,255,255,0.03)', color: '#ca8a04', border: '1px solid rgba(255,255,255,0.08)' }}>
                <m.icon size={11} /> {m.type}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <div className="text-xs" style={{ color: '#64748b' }}>
                <span className="font-semibold" style={{ color: 'rgba(255,255,255,0.04)' }}>{m.date}</span> · {m.time}
              </div>
              <motion.button whileTap={{ scale: 0.97 }} className="btn-gold flex items-center gap-1.5 px-3 py-1.5 text-xs">
          ...
          return (
            <div key={m.title} className="flex items-center px-5 py-4 lead-row"
              ...
              <div className="w-9 h-9 rounded-xl flex items-center justify-center mr-4 flex-shrink-0" style={{ background: 'rgba(255,255,255,0.03)' }}>
                <m.icon size={15} style={{ color: '#ca8a04' }} />
              </div>
              <div className="flex-1">
                <p className="text-sm font-semibold" style={{ color: 'rgba(255,255,255,0.04)' }}>{m.title}</p>
                <p className="text-xs" style={{ color: '#64748b' }}>{m.company}</p>
              </div>
